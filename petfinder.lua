-- Pet Scanner v2 - Full Checklist + Auto Scan + Server Hop
...
local autoHop = true  -- always on by default
local hopCooldown = false
local checkedPets = loadTargets()  -- load from file
...
-- Pet checklist scroll
local petScroll = Instance.new("ScrollingFrame", main)
...
local petScrollLayout = Instance.new("UIListLayout", petScroll)
petScrollLayout.Padding = UDim.new(0, 3)
petScrollLayout.SortOrder = Enum.SortOrder.LayoutOrder
local petScrollPad = Instance.new("UIPadding", petScroll)
...
for _, petName in ipairs(ALL_PETS) do
    local petRow = Instance.new("Frame", petScroll)
    ...
    local petNameLbl = Instance.new("TextLabel", petRow)
    ...
    for si, size in ipairs(ALL_SIZES) do
        ...
        local checkFrame = Instance.new("Frame", petRow)
        checkFrame.Size = UDim2.new(0, 70, 0, 20)
        checkFrame.Position = UDim2.new(0, (si-1)*78 + 8, 0, 28)
        ...
        local checkLbl = Instance.new("TextLabel", checkFrame)
        ...
        checkBtn.MouseButton1Click:Connect(function()
            ...
            _G.PetScannerTargets = checkedPets
            saveTargets(checkedPets)  -- save to file
...
-- Current pets in server
local currentLbl = Instance.new("TextLabel", main)
...
local currentFrame = Instance.new("ScrollingFrame", main)
...
local currentLayout = Instance.new("UIListLayout", currentFrame)
currentLayout.Padding = UDim.new(0, 3)
local currentPad = Instance.new("UIPadding", currentFrame)
...
local function refreshCurrentPets()
    ...
    local pets = getPets()
    if #pets == 0 then
        local empty = Instance.new("TextLabel", currentFrame)
    ...
    for _, pet in pairs(pets) do
        ...
        local row = Instance.new("Frame", currentFrame)
...
-- If coming from a server hop, set hop button state
